...
function DataGridTableContainer({ children }: { children: ReactNode }) {
  const { table, isLoading } = useDataGrid();
  const isEmpty = !isLoading && table.getRowModel().rows.length === 0;
  const shouldWrapWithScrollArea =
    React.isValidElement(children) && children.type !== DataGridScrollArea;

  return (
    <div
      className={cn(
        "w-full overflow-hidden",
        !isEmpty && "rounded-md border border-border bg-background",
      )}
    >
      {shouldWrapWithScrollArea ? (
        <DataGridScrollArea>{children}</DataGridScrollArea>
      ) : (
        children
      )}
    </div>
...
function DataGridTableBase({ children }: { children: ReactNode }) {
  ...
  return (
    <table
      ...
      className={cn(
        "min-w-full w-full align-middle caption-bottom text-left rtl:text-right text-foreground font-normal text-sm",
...
function DataGridTableHeadRowCell<TData>({
  ...
  const fillRemaining = column.columnDef.meta?.fillRemaining === true;
  const widthStyles = fillRemaining
...
function DataGridTableBodyRow<TData>({
  ...
  // Skip animations if motion should be reduced
...
function DataGridTableBodyRowCell<TData>({
  ...
  const fillRemaining = column.columnDef.meta?.fillRemaining === true;
  const widthStyles = fillRemaining ? getCommonPinningStyles({ column }) : {};
...
function DataGridTableEmpty() {
  ...
  return (
    <tr className="[&>td]:border-0 [&>td]:bg-background">
      <td colSpan={totalColumns} className="text-center text-muted-foreground">
...
function DataGridTable<TData>() {
  ...
  const showHeaders =
    hasRows ||
    (isLoading &&
      (props.loadingMode === "skeleton" || props.loadingMode === "spinner"));
