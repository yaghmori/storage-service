...
export const files = pgTable(
  ...
  (table) => ({
    ...
    providerIdx: index('files_storage_provider_idx').on(
      table.storageProviderId
    ),
    ...
    processingStatusIdx: index('files_processing_status_idx').on(
...
export const fileVariants = pgTable(
  ...
  (table) => ({
    ...
    providerIdx: index('file_variants_storage_provider_idx').on(
      table.storageProviderId
    ),
    // Composite index for common queries
    fileVariantIdx: index('file_variants_file_type_idx').on(
      table.fileId,
      table.variantType
    ),
    // Check constraints
...
export const processingJobs = pgTable(
  ...
  (table) => ({
    ...
    bullmqJobIdIdx: index('processing_jobs_bullmq_job_id_idx').on(
      table.bullmqJobId
    ),
    // Composite index for common queries
    fileStatusIdx: index('processing_jobs_file_status_idx').on(
      table.fileId,
      table.status
    ),
    // Check constraints
...
export const downloadLogs = pgTable(
  ...
  (table) => ({
    ...
    downloadedAtIdx: index('download_logs_downloaded_at_idx').on(
      table.downloadedAt
    ),
    ...
    fileDateIdx: index('download_logs_file_date_idx').on(
      table.fileId,
      table.downloadedAt
    ),
  })
...
export const fileDuplicates = pgTable(
  ...
  (table) => ({
    // Indexes for performance
    originalFileIdx: index('file_duplicates_original_idx').on(
      table.originalFileId
    ),
    detectedAtIdx: index('file_duplicates_detected_at_idx').on(
      table.detectedAt
    ),
    uploadedByIdx: index('file_duplicates_uploaded_by_idx').on(
      table.uploadedBy
    ),
